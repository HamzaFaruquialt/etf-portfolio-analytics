...
Pulls the data. 10 years of daily prices for all 8 ETFs from Yahoo Finance.
Cleans it and computes returns. Daily returns, cumulative growth, and rolling volatility, on which everything downstream is built.
Loads it into SQLite with a real typed schema (not just pandas dumping a table), and runs some of the analysis directly in SQL. There's a Sharpe ratio leaderboard and a max-drawdown calc done with window functions, which I then cross-checked against the pandas version to make sure they actually agree.
Checks how much each ETF moves with the others. This is the diversification question. If two ETFs move together, holding both doesn't really lower your risk.
Finds the "best" portfolio mix using Markowitz optimization (the actual math behind "don't put all your eggs in one basket"). I solve for both the lowest-risk mix and the best risk-adjusted-return mix.
Asks "how bad could it get?" with Monte Carlo simulation plus a historical-resampling method to estimate how much the portfolio could lose on a rough day (Value-at-Risk / Conditional VaR).
Tests whether the optimization would've actually worked in real life. This is the part I think matters most. Instead of optimizing over all 10 years at once and declaring victory (which is cheating, since you'd never have known the future), I rerun the optimization every year using only the data available up to that point and see how it would've actually performed going forward.
...
Chart.js for the live web dashboard
...
I built two views of the same results on purpose. The live web dashboard is the full interactive deep-dive with every chart and the headline findings written out. The Tableau Public version is the concise one, the four charts that tell the story at a glance: cumulative performance, risk-return, the correlation matrix, and the efficient frontier.
